...
def main():
    ...
    for e in range(epoch):
        ...
        #Model will be trained in all Processor
        model.fit(x_train, y_train, batch_size=32, epochs=1, verbose=0)
        comm.Barrier()

        # Dederated step in Decentralized FL
        #data contain number of sample and weight in each processor
        data = [x_train.shape[0], model.get_weights()]

        #In this step we use many-to-many communication pattern, all processors can update the weight from all other workers
